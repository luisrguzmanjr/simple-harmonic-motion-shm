...
let mySprite4 = sprites.create(img`
    ...
    . . . . . . . . . . . . . . . . 
    `, SpriteKind.Enemy)
let baseAngle = 1.5 * Math.PI
let halfAngleRange = 0.25 * Math.PI
let rate = 40
let cx = 40
game.onUpdate(function () {
    time = game.runtime() / 10000
    mySprite.x = screen.width / 2 + cx * Math.cos(rate * time)
    mySprite.y = screen.height / 2 + cx * Math.sin(rate * time)
})
game.onUpdate(function () {
    c = Math.cos(rate * time)
    angle = halfAngleRange * c + baseAngle
    mySprite3.x = screen.width / 2 + cx * Math.cos(angle)
    mySprite3.y = screen.height / 2 + cx * Math.sin(angle)
})
game.onUpdate(function () {
    time = game.runtime() / 10000
